...
TcpsEcallDemux(
    ...
    TEE_Param params[4])
{
    const uint8_t* in_buffer = (const uint8_t*)params[0].memref.buffer;
    uint32_t in_buffer_size = params[0].memref.size;
    uint8_t* out_buffer = (uint8_t*)params[1].memref.buffer;
    uint32_t out_buffer_size = params[1].memref.size;
    uint32_t rpcKey;
    TEE_Result result;
    ...
    result = TcpsGetParameters(param_types, params, &rpcKey, (void**)&in_buffer, &in_buffer_size,
                               (void**)&out_buffer, &out_buffer_size);
    if (result != TEE_SUCCESS) {
