...
**  This function composes the valid OE_EXCEPTION_RECORD, and call the
*registered
**  exception handlers one by one. If one handler returns
*OE_EXCEPTION_CONTINUE_EXECUTION,
**  this function will continue execution on the context. Otherwise the enclave
*will
...
void _OE_ExceptionDispatcher(OE_CONTEXT* oe_context)
{
    ...
    // N.B, In second pass exception handling, the XSTATE are recovered by SGX
    // hardware
    // correctly on ERSUME, our exception code doesn't touch them, so that we
    // don't
    ...
    // Traversal the existing exception handlers, stop when
...
**  is an exception in current enclave thread, and save minimal exception
*context
...
void _OE_VirtualExceptionDispatcher(TD* td, uint64_t argIn, uint64_t* argOut)
{
    ...
    // running, and let enclave handle
    // the exception.
