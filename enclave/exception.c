    ...
    POE_VECTORED_EXCEPTION_HANDLER vectoredHandler)
{
    ...
    // Sanity check.
    if (vectoredHandler == NULL || !OE_IsWithinEnclave((void*)vectoredHandler, 8))
    {
        goto cleanup;
    }
    ...
    if (isFirstHandler == 0)
    {
        ...
        g_exception_handler_arr[g_current_exception_handler_count] = vectoredHandler;
    }
    else
    {
    ...
    void* vectoredHandler)
{
    ...
    for (uint32_t i = 0; i < g_current_exception_handler_count; i++)
    {
        ...
        // Found the target handler, move the following handlers forward by one if any.
    ...
    uint64_t* argOut)
{
    ...
    for (uint32_t i = 0; i < OE_COUNTOF(g_vector_to_exception_code_mapping); i++)
    {
        ...
        {
            td->base.exception_code = g_vector_to_exception_code_mapping[i].exception_code;
            break;
        }
    }
