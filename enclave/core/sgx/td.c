...
static oe_sgx_td_t* _sgx_get_td(bool check_fs)
{
    ...
    if (fsbase != gsbase)
    {
         ...
         * Currently, the only place that needs to bypass the check is the
         * second-stage exception handling as we cannot the application to
        ...
        if (check_fs)
        {
            ...
            {
                /* Instead of calling oe_abort which also depends on
                 * oe_sgx_get_td, set the td state to ABORTED and call
                 * oe_asm_exit to exit the enclave. The subsequent enclave
                 * entries will then be blocked */
