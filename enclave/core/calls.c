...
/**
 * The _oe_ecalls_table table is expected to be generated by oeedger8r
 */
...
static oe_result_t _handle_call_enclave_function(uint64_t arg_in)
{
    ...
    // Call the function.
    // Note: Currently only SGX style marshaling is supported.
    // args.input_buffer contains the marshaling struct.
    f(args.input_buffer);
