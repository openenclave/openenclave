...
#define __oe_ecalls_table __oe_optee_ecalls_table
#define __oe_ecalls_table_size __oe_optee_ecalls_table_size
    ...
    size_t* output_bytes_written)
{
    return oe_call_host_function_by_table_id(
        OE_OPTEE_OCALL_FUNCTION_TABLE_ID,
        function_id,
        input_buffer,
        input_buffer_size,
        output_buffer,
...
/* Registers the optee ECALL function table. */
oe_result_t oe_register_optee_ecall_function_table(void)
{
    const uint64_t table_id = OE_OPTEE_ECALL_FUNCTION_TABLE_ID;
    const oe_ecall_func_t* ecalls = __oe_optee_ecalls_table;
    const size_t num_ecalls = __oe_optee_ecalls_table_size;
