...
docker build -f Dockerfile.oe-deb --build-arg BASE_IMAGE=$BUILD_BASE_IMAGE --build-arg BUILD_USER=$BUILD_USER --build-arg BUILD_USER_ID=$BUILD_USER_ID --build-arg BUILD_USER_HOME=$BUILD_USER_HOME --no-cache . -t openenclave-sample-build
