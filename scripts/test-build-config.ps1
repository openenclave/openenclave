...
if ($ADD_WINDOWS_ENCLAVE_TESTS) {
    & cmake.exe -G $BUILD_GENERATOR $LINUX_BIN_FLAG -DADD_WINDOWS_ENCLAVE_TESTS=1 $BUILD_ENCLAVES_FLAG ..
} else {
    & cmake.exe -G $BUILD_GENERATOR $LINUX_BIN_FLAG $BUILD_ENCLAVES_FLAG ..
}
if ($LASTEXITCODE) {
    echo ""
    echo "cmake failed"
