    ...
    uint64_t einittoken)
{
    ...
    uint8_t zerohash[OE_SHA256_SIZE];
    memset(zerohash, 0, OE_SHA256_SIZE);

    /* Ask driver to perform EINIT */
    if (memcmp(((SGX_SigStruct*)sigstruct)->enclavehash, zerohash, OE_SHA256_SIZE) == 0)
    {
printf("*** ZERO-SIGSTRUCT\n");
        OE_SHA256 hash;
        OE_TRY(self->measurer->gethash(self->measurer, &hash));

        SGX_SigStruct ss;
        OE_TRY(OE_SignEnclave(&hash, (const uint8_t*)KEY, sizeof(KEY), &ss));
...
__SGX_DumpSigStruct(&ss);
        if (_Ioctl(self, SGX_IOC_ENCLAVE_INIT, &param) != 0)
            OE_THROW(OE_IOCTL_FAILED);
    }
    else
    {
printf("*** NONZERO-SIGSTRUCT\n");
        SGXEinitParam param;
...
printf("SIGSTRUCT.MRENCLAVE: ");
OE_HexDump(((SGX_SigStruct*)sigstruct)->enclavehash, OE_SHA256_SIZE);

printf("ACTUAL.MRENCLAVE: ");
OE_SHA256 hash;
OE_TRY(self->measurer->gethash(self->measurer, &hash));
OE_HexDump(&hash, OE_SHA256_SIZE);
