...
static void _HostSignalHandler(int sigNum, siginfo_t* sigInfo, void* sigData)
{
    ...
    else if (g_previous_sigaction[sigNum].sa_handler == SIG_DFL)
    {
        ...
        // set, raise it again, and let the
    ...
    else
    {
        ...
        // transfer the signal to the old
        // signal handler;
        ...
        // which means
        // g_previous_sigaction->next_old_sigact will not be called. Our signal
        // handler does not
...
static void _RegisterSignalHandlers(void)
{
    ...
    // To use sa_sigaction instead of sa_handler, and allow catch the same
    // signal as the one you're currently handling,
