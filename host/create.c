    ...
    uint64_t* vaddr)
{
    ...
    {
        const oe_page* pages = (const oe_page*)reloc_data;
        size_t npages = reloc_size / sizeof(oe_page);
    ...
    uint64_t* vaddr)
{
    ...
    {
        const oe_page* pages = (const oe_page*)ecall_data;
        size_t npages = ecall_size / sizeof(oe_page);
    ...
    oe_enclave_t* enclave)
{
    ...
    /* Patch the "__oe_base_reloc_page" */
    {
        Elf64_Sym sym;

        if (Elf64_FindDynamicSymbolByName(elf, "__oe_base_reloc_page", &sym) != 0)
            OE_RAISE(OE_FAILURE);

        *(uint64_t*)((uint8_t*)segpages + sym.st_value) = base_reloc_page;
    }

    /* Patch the "__oe_num_reloc_pages" */
    {
        Elf64_Sym sym;

        if (Elf64_FindDynamicSymbolByName(elf, "__oe_num_reloc_pages", &sym) != 0)
            OE_RAISE(OE_FAILURE);

        *(uint64_t*)((uint8_t*)segpages + sym.st_value) =
            reloc_size / OE_PAGE_SIZE;
    }

    /* Patch the "__oe_base_ecall_page" */
    {
        Elf64_Sym sym;

        if (Elf64_FindDynamicSymbolByName(elf, "__oe_base_ecall_page", &sym) != 0)
            OE_RAISE(OE_FAILURE);

        *(uint64_t*)((uint8_t*)segpages + sym.st_value) = base_ecall_page;
    }

    /* Patch the "__oe_num_ecall_pages" */
    {
        Elf64_Sym sym;

        if (Elf64_FindDynamicSymbolByName(elf, "__oe_num_ecall_pages", &sym) != 0)
            OE_RAISE(OE_FAILURE);

        *(uint64_t*)((uint8_t*)segpages + sym.st_value) =
            ecall_size / OE_PAGE_SIZE;
    }

    /* Patch the "__oe_base_heap_page" */
    {
        Elf64_Sym sym;

        if (Elf64_FindDynamicSymbolByName(elf, "__oe_base_heap_page", &sym) != 0)
            OE_RAISE(OE_FAILURE);

        *(uint64_t*)((uint8_t*)segpages + sym.st_value) = base_heap_page;
    }

    /* Patch the "__oe_num_heap_pages" */
    {
        Elf64_Sym sym;

        if (Elf64_FindDynamicSymbolByName(elf, "__oe_num_heap_pages", &sym) != 0)
            OE_RAISE(OE_FAILURE);

        *(uint64_t*)((uint8_t*)segpages + sym.st_value) = nheappages;
    }

    /* Patch the "__oe_num_pages" */
    {
        Elf64_Sym sym;
        uint64_t npages = enclave_end / OE_PAGE_SIZE;

        if (Elf64_FindDynamicSymbolByName(elf, "__oe_num_pages", &sym) != 0)
            OE_RAISE(OE_FAILURE);

        *(uint64_t*)((uint8_t*)segpages + sym.st_value) = npages;
    }

    /* Patch the "__oe_virtual_base_addr" */
    {
        Elf64_Sym sym;

        if (Elf64_FindDynamicSymbolByName(elf, "__oe_virtual_base_addr", &sym) !=
            0)
        {
            OE_RAISE(OE_FAILURE);
        }
...
static oe_result_t _build_ecall_array(oe_enclave_t* enclave, Elf64* elf)
{
    oe_result_t result = OE_UNEXPECTED;
    Elf64_Shdr shdr;
...
static oe_result_t _save_text_address(oe_enclave_t* enclave, Elf64* elf)
{
    oe_result_t result = OE_UNEXPECTED;
    Elf64_Shdr shdr;
    ...
    oe_sgx_enclave_properties_t* properties)
{
    ...
    if (Elf64_FindSection(elf, section_name, &section_data, &section_size) != 0)
    {
        result = OE_NOT_FOUND;
        goto done;
    }
    ...
    const char** field_name)
{
    ...
    {
        if (field_name)
            *field_name = "header.sizeSettings.numHeapPages";
        result = OE_FAILURE;
        goto done;
    }
    ...
    {
        if (field_name)
            *field_name = "header.sizeSettings.numStackPages";
        result = OE_FAILURE;
        goto done;
    }
    ...
    {
        if (field_name)
            *field_name = "header.sizeSettings.numTCS";
        result = OE_FAILURE;
        goto done;
    }
    ...
    {
        if (field_name)
            *field_name = "config.securityVersion";
        result = OE_FAILURE;
        goto done;
    }
    ...
    oe_enclave_t* enclave)
{
    ...
    if (Elf64_LoadRelocations(&elf, &reloc_data, &reloc_size) != 0)
        OE_RAISE(OE_FAILURE);
