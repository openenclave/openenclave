    ...
    uint64_t numTCS)
{
    ...
    const oe_sgx__enclave_config_t* config = &props->config;

    /* Check the header */
    OE_TEST(header->size == sizeof(oe_sgx__enclave_properties_t));
...
int main(int argc, const char* argv[])
{
    ...
    {
        oe_puterr("oe_sgx__load_properties(): result=%u", result);
    }
