...
OE_ECALL void WaitForExclusiveAccess(void* args_)
{
    ...
    while (nthreads > 0)
    {
        ...
        oe_host_printf("%ld: Waiting for exclusive access\n", oe_thread_self());
        oe_cond_wait(&exclusive, &ex_mutex);
    }

    oe_host_printf("%ld: Obtained exclusive access\n", oe_thread_self());
    nthreads = 1;
...
OE_ECALL void RelinquishExclusiveAccess(void* args_)
{
    ...
    oe_host_printf("%ld: Signalling waiting threads\n", oe_thread_self());
    oe_cond_signal(&exclusive);

    oe_host_printf("%ld: Relinquished exlusive access\n", oe_thread_self());
    oe_mutex_unlock(&ex_mutex);
}
