    ...
    int is_on_windows)
{
    ...
    if (is_on_windows && (!is_misc_region_supported || !pfgp_opt_in))
        return 2;

    /* For SGX1 enclaves or SGX2 enclaves with CapturePFGPExceptions=0,
     * the PF simulation is only supported in debug mode */
    if ((!is_misc_region_supported || !pfgp_opt_in) &&
     ...
     * cleared in non-simulation case (SGX2). In debug mode (SGX1 or
     * SGX2 with CapturePFGPExceptions=0), the faulting address will always
     * be page-aligned (passed by the host). */
    faulting_address = 0;
    bypass_bytes = MOV_INSTRUCTION_BYTES;
    ...
    else
    {
        /* faulting_address is passed in by the host */
        OE_TEST(faulting_address == 0);
        /* In debug mode (SGX1 or SGX2 with CapturePFGPExceptions=0),
         * simulating all the unsupported exception types as #PF. */
        OE_TEST(exception_code == OE_EXCEPTION_PAGE_FAULT);
        OE_TEST(error_code == OE_SGX_PAGE_FAULT_US_FLAG);
    }
