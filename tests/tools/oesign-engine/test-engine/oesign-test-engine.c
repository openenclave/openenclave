...
#if 0
static EVP_PKEY_METHOD *test_engine_rsa = NULL;
#endif
...
/*
 * We pass the name of the pem file as the keyid, so that we can actually use the signing key from the openenclave build 
 * as a signing key for oesign from the test engine. We know the test went well when we can load a test enclave
 */
static EVP_PKEY *get_private_key_from_keyid(const char *keyid)
{
     ...
     const char *path = keyid;  // Do we wanty to directly do that or have an environment variable to direct to a path for pem files?
...
static EVP_PKEY *test_engine_load_privkey(ENGINE *e, const char *keyid,
                                         UI_METHOD *ui_method,
                                         void *callback_data)
{
...
static EVP_PKEY *test_engine_load_pubkey(ENGINE *e, const char *keyid,
                                         UI_METHOD *ui_method,
                                         void *callback_data)
{
     (void)ui_method;
...
#if 0
static int test_engine_pkey_meths(ENGINE *e, EVP_PKEY_METHOD **pmeth,
...
#if 0
    if (!ENGINE_set_digests(e, test_engine_digests)) {
        printf("ENGINE_set_digests failed\n");
        goto done;
    }
    if (!ENGINE_set_ciphers(e, test_engine_ciphers)) {
        printf("ENGINE_set_ciphers failed\n");
        goto done;
    }
    if (!ENGINE_set_pkey_meths(e, test_engine_pkey_meths)) {
        printf("ENGINE_set_pkey_meths failed\n");
        goto done;
    }
    if (!ENGINE_set_pkey_asn1_meths(e, test_engine_pkey_asn1_meths)) {
        printf("ENGINE_set_pkey_asn1_meths failed\n");
        goto done;
    }

    /* Control function and commands */
    if (!ENGINE_set_cmd_defns(e, test_engine_cmds)) {
        fprintf(stderr, "ENGINE_set_cmd_defns failed\n");
        goto done;
    }
    if (!ENGINE_set_ctrl_function(e, test_engine_control_func)) {
