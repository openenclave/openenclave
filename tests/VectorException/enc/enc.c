...
int VectorExceptionSetup()
{
    ...
    {
        handler = OE_AddVectoredExceptionHandler(0, g_test_pass_through_handlers[i]);
        if (handler == NULL)
        {
            return ret;
        }
    ...
    for (uint32_t i = 0; i < OE_COUNTOF(g_test_pass_through_handlers); i++)
    {
        ret = OE_RemoveVectoredExceptionHandler(g_test_pass_through_handlers[i]);
        if (ret != 0)
        {
            return ret;
        }
    ...
    for (uint32_t i = 0; i < OE_COUNTOF(g_test_pass_through_handlers); i++)
    {
        handler = OE_AddVectoredExceptionHandler(1, g_test_pass_through_handlers[i]);
        if (handler == NULL)
        {
            return ret;
        }
...
OE_ECALL void TestVectorException(void* args_)
{
    ...
    OE_HostPrintf("TestVectorException: will generate a hardware exception inside enclave!\n");
    ...
    OE_HostPrintf("TestVectorException: hardware exception is handled correctly!\n");
