...
{
    for (uint32_t seal_policy = OE_SEAL_ID_UNIQUE;
         seal_policy <= OE_SEAL_ID_PRODUCT;
         seal_policy++)
    {
        ...
        ret = oe_get_seal_key_by_policy(
            (oe_seal_id_policy_t)seal_policy, NULL, NULL, NULL, NULL);
        ...
        uint32_t second_key_buffer_size = sizeof(second_key_buffer);
        uint8_t key_info[sizeof(sgx_key_request_t)] = {0};
        uint32_t key_info_size = sizeof(key_info);
        ...
        uint32_t third_key_buffer_size = sizeof(third_key_buffer);
