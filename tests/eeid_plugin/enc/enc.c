...
#include "../plugin/eeid_attester.h"
#include "../plugin/eeid_plugin.h"
#include "../plugin/eeid_verifier.h"
#include "../plugin/test_helpers.h"
...
static void _test_evidence_success(const oe_uuid_t* format_id)
{
    ...
    size_t h_endorsements_size =
        sizeof(header_t) + sizeof(oe_eeid_t) + eeid->data_size;
    header_t* h_endorsements = malloc(h_endorsements_size);
    h_endorsements->version = 1;
    h_endorsements->format_id = plugin_uuid;
    h_endorsements->data_size = eeid_size;
    memcpy(h_endorsements->data, eeid, eeid_size);

    OE_TEST(
        oe_verify_evidence(
            evidence,
            evidence_size,
            (uint8_t*)h_endorsements,
            h_endorsements_size,
            NULL,
            0,
            &claims,
