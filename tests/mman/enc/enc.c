...
static void _test_basic()
{
    ...
    uint8_t* ptr = (uint8_t*)mmap(
        NULL, 1024, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
...
static void _test_mmap_params(void)
{
    ...
    OE_TEST(
        mmap((void*)1, 1024, PROT_READ, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0) ==
        MAP_FAILED);
    ...
    OE_TEST(
        mmap(NULL, (1L << 52), PROT_READ, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0) ==
        MAP_FAILED);
    ...
    // Test various prots.
    OE_TEST(
        mmap(NULL, 1024, 0, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0) == MAP_FAILED);
    OE_TEST(errno == EINVAL);

    OE_TEST(
        mmap(NULL, 1024, PROT_EXEC, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0) ==
        MAP_FAILED);
    OE_TEST(errno == EINVAL);

    OE_TEST(
        mmap(NULL, 1024, PROT_READ, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0) !=
        MAP_FAILED);
    OE_TEST(errno == 0);

    OE_TEST(
        mmap(NULL, 5024, PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0) !=
        MAP_FAILED);
    OE_TEST(errno == 0);
    ...
    // Test various flags
    OE_TEST(mmap(NULL, 1024, PROT_READ | PROT_WRITE, 0, -1, 0) == MAP_FAILED);
    OE_TEST(errno == EINVAL);

    // One of MAP_SHARED, MAP_SHARED_VALIDATE, MAP_PRIVATE must be used.
    OE_TEST(
        mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_SHARED, -1, 0) !=
        MAP_FAILED);
    OE_TEST(errno == 0);

    OE_TEST(
        mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_SHARED_VALIDATE, -1, 0) !=
        MAP_FAILED);
    OE_TEST(errno == 0);

    OE_TEST(
        mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_PRIVATE, -1, 0) !=
        MAP_FAILED);
    OE_TEST(errno == 0);

    OE_TEST(
        mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_ANONYMOUS, -1, 0) ==
        MAP_FAILED);
    OE_TEST(errno == EINVAL);

    // TEst unsupported flags.
    OE_TEST(
        mmap(NULL, 1024, PROT_READ, MAP_ANONYMOUS | MAP_PRIVATE, 0, 0) !=
        MAP_FAILED);
    OE_TEST(errno == 0);
    ...
    {
        OE_TEST(
            mmap(NULL, 1024, PROT_READ, unsupported[i] | MAP_PRIVATE, 0, 0) ==
            MAP_FAILED);
        OE_TEST(errno == EINVAL);
    }
    ...
    {
        OE_TEST(
            mmap(NULL, 1024, PROT_READ, ignored[i] | MAP_PRIVATE, 0, 0) !=
            MAP_FAILED);
        OE_TEST(errno == 0);
    }
    ...
    OE_TEST(
        mmap(NULL, 1024, PROT_READ, MAP_ANONYMOUS | MAP_PRIVATE, 1234, 0) !=
        MAP_FAILED);
    OE_TEST(errno == 0);

    // offset must be zero.
    OE_TEST(
        mmap(NULL, 1024, PROT_READ, MAP_ANONYMOUS | MAP_PRIVATE, 0, -1) ==
        MAP_FAILED);
    OE_TEST(errno == EINVAL);
...
static void _test_unmap_params(void)
{
    ...
    OE_TEST(munmap((void*)1024, OE_PAGE_SIZE) != 0);
    OE_TEST(errno == EINVAL);
}
