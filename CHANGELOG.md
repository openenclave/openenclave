...
### Changed
- OID values supplied for custom X.509 extensions are now required to use
  strict DER sub-identifier encoding.
...
- OE SDK can now be built with Clang-11 and it is recommended to upgrade the compiler to Clang-11 if you are building the SDK from source. 
  - Building OE SDK enclaves now includes the following LVI mitigation options:
    ...
    - *Note*: The packages released in 0.19.0 are built with ControlFlow-GNU mitigations
...
- Open Enclave SDK is no longer tested on Ubuntu 18.04. Ubuntu 18.04 is at EOL. v0.19.0 will not contain packages for Ubuntu 18.04.
