...
typedef OE_Result (*OE_ECreateProc)(OE_SGXDevice* dev, uint64_t enclaveSize, uint64_t* enclaveAddr);

typedef OE_Result (
    *OE_EAddProc)(OE_SGXDevice* dev, uint64_t base, uint64_t addr, uint64_t src, uint64_t flags, bool extend);

typedef OE_Result (*OE_EInitProc)(OE_SGXDevice* dev, uint64_t addr, uint64_t sigstruct, uint64_t einittoken);
