...
function (add_enclave_test TEST_NAME HOST_FILE ENC_FILE)
  ...
  if (ADD_WINDOWS_ENCLAVE_TESTS)
    ...
    string(FIND ${CMAKE_CURRENT_SOURCE_DIR} ${OE_TESTS_PATH_NAME}
                TEST_DIR_INDEX)

    if (${TEST_DIR_INDEX} GREATER -1)
      math(EXPR TEST_DIR_INDEX "${TEST_DIR_INDEX}+${OE_TESTS_PATH_NAME_LENGTH}")
      string(SUBSTRING ${CMAKE_CURRENT_SOURCE_DIR} ${TEST_DIR_INDEX} -1
                       TEST_DIR)
    endif ()
    ...
    if (${TEST_DIR_INDEX} LESS_EQUAL -1 OR TEST_DIR STREQUAL "")
      message(
        FATAL_ERROR
          "add_enclave_test can only be used in a subfolder of ${OE_TESTS_PATH_NAME} with ADD_WINDOWS_ENCLAVE_TESTS."
      )
    endif ()
    ...
    add_test(
      NAME ${TEST_NAME}
      COMMAND
        $<TARGET_FILE:${HOST_FILE}>
        ${CMAKE_CURRENT_BINARY_DIR}/${TEST_ENCSUBPATH}/${TEST_ENCFILE} ${ARGN})
    ...
    if (LVI_MITIGATION MATCHES ControlFlow AND NOT LVI_MITIGATION_SKIP_TESTS)
      ...
      if (TEST_ENCFILE_LVI MATCHES ".*\.signed")
        string(REPLACE ".signed" "-lvi-cfg.signed" TEST_ENCFILE_LVI
                       "${TEST_ENCFILE_LVI}")
      else ()
      ...
      add_test(
        ...
        COMMAND
          $<TARGET_FILE:${HOST_FILE}>
          ${CMAKE_CURRENT_BINARY_DIR}/${TEST_ENCSUBPATH}/${TEST_ENCFILE_LVI}
          ${ARGN})
    endif ()

  elseif (UNIX OR USE_CLANGW)
    if (OE_SGX)
      add_test(NAME ${TEST_NAME} COMMAND $<TARGET_FILE:${HOST_FILE}>
                                        $<TARGET_FILE:${ENC_FILE}> ${ARGN})
      if (LVI_MITIGATION MATCHES ControlFlow AND NOT LVI_MITIGATION_SKIP_TESTS)
        set(TEST_NAME_LVI "${TEST_NAME}-lvi-cfg")
        ...
        add_test(NAME ${TEST_NAME_LVI}
                COMMAND $<TARGET_FILE:${HOST_FILE}>
                        $<TARGET_FILE:${ENC_FILE_LVI}> ${ARGN})
      endif ()
    elseif (OE_TRUSTZONE)
      get_target_property(ENCLAVE_UUID ${ENC_FILE} ENCLAVE_UUID)
      add_test(NAME ${TEST_NAME} COMMAND $<TARGET_FILE:${HOST_FILE}> ${ENCLAVE_UUID} ${ARGN})
    endif ()
  endif ()
