...
int main(int argc, const char* argv[])
{
    ...
    /* Deduce enclave path from host path */
    {
        ...
        snprintf(path, sizeof(path), "%s/tlssrv_enclave", root);

        free(clone);
    }

    r = oe_create_tlssrv_enclave(path, type, flags, NULL, 0, &enclave);
    if (r != OE_OK)
    {
    ...
    r = tlssrv_run_server_ecall(enclave, &retval);
    if (r != OE_OK || retval != 0)
    {
        fprintf(stderr, "%s: failed to run server\n", argv[0]);
        exit(1);
    }
