...
### Goals
1. Consolidate SGX_ECDSA_P256 format IDs.
2. Deprecate oe_report definitions.
...
For all evidence structures above, the hearts of the evidences are the same - SGX ECDSA_P256 quote. SGX ECDSA_P256 quote should to be like any other evidence format which is presented in 2 formats - with or without embedded format id. For example, format XYZ is presented as:
...
In the proposal, flag `OE_EVIDENCE_FLAGS_EMBED_FORMAT_ID` indicates wraping the plugin evidence with a header containing format ID, and hashing the custom claims and passing the hash to the plugin.

Consider renaming the flag to reflect the fact.
...
### Plan
Reduction/deprecation is planned in phases.

Here 'Remove' means the API definition is removed from public header files. Referring to the removed API definition results in compilation error.
'Deprecate' means the API definition is redefined in public header files. Referring to the deprecated API definition does not break the build but the behavior is changed internally to guarantee continuous support.

#### Phase 1 - V0.11
- Remove `OE_FORMAT_UUID_SGX_ECDSA_P256_QUOTE` as proposed above.
...
At the end of phase 2, oe_get_evidence() and oe_verify_evidence() support these evidence formats:

| Format ID | Evidence structure  |
| -- | - |
| A. `OE_FORMAT_UUID_SGX_ECDSA_P256` | `SGX_ECDSA_quote(custom_claims)` |
| B. `OE_FORMAT_UUID_SGX_ECDSA_P256 + flag` | `oe_attestation_header \|\| SGX_ECDSA_quote(hash) \|\| custom_claims` |
| C. `OE_FORMAT_UUID_SGX_ECDSA_P256_REPORT` | `oe_report_header \|\| SGX_ECDSA_quote(custom_claims)` |

#### Phase 3 - V0.12
- Deprecate `oe_get_report`.
- Deprecate `oe_verify_report`.
- Remove `oe_verify_attestation_certificate_with_evidence`.

#### Phase 4 - Future
- Deprecate OE_FORMAT_UUID_SGX_ECDSA_P256_REPORT.

At the end of phase 4, only format A and B are supported.
