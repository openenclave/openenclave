...
int main(int argc, const char* argv[])
{
    ...
    if (argc != 3)
    {
        fprintf(
            stderr,
            "Usage: %s enclave_image_path enc_log_abs_filepath [ --simulate  "
            "]\n",
            argv[0]);
        goto exit;
    }
    ...
    fprintf(
        stdout,
        "hostfs.c:_hostfs_ioctl:988 - error code 25 expected."
        " The program will continue uninterrupted \n");
    result = enclave_set_log_callback(enclave, argv[2]);
...
exit:
    result = enclave_hostfs_unmount(enclave);
    if (enclave)
