...
oe_get_seal_key_by_policy. This method is used to get the sealing key to unseal previously sealed data. This is recommended because events such as patching of the server can change the properties used to derive the sealing key (e.g. CPUSVN) in oe_get_seal_key_by_policy. As a best practice, you should persist the keyInfo along with your encrypted data for such scenarios.
...
- enclave_a_v1, and enclave_a_v2 were created and signed by the same private.pem file, which means same signer.
...
- enclave_b was signed by a newly created private.mem, a different signer/product.
...
- The host unseals the sealed_data_t in the enclave_a_v1 and expects to see it's unsealed **successfully** because this sealed data was sealed in the same enclave_a_v1 enclave.
...
- seal_data calls oe_get_seal_key_by_policy with OE_SEAL_POLICY_UNIQUE/PRODUCT to get a unique seal key and its seal keyinfo.
- generate initialization vector
- Encrypt input data
- Allocate sealed_data_t structure
- Generate signature from the sealed_data_t structure with the seal key 
- Fill sealed_data_t with above information before returning to the host
...
- seal_data calls oe_get_seal_key with keyinfo from sealed_data_t
- retrieve initialization vector from sealed_data_t.iv
- re-generate a new signature from sealed_data_t and validate it against sealed_data_t.signature
- decrypt sealed_data_t.encrypted_data
- Return decrypted data before returning to the host
