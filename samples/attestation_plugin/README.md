...
## About the attestation_plugin sample
...
First we need to define the functions we want to call between the enclave and host. To do this we create a `attestation_plugin.edl` file:
...
In this `attestation_plugin.edl` file we define two different functions.

```c
public void enclave_attestation_plugin();
```
...
```bash
oeedger8r ../attestation_plugin.edl --untrusted
```

This command compiles the `attestation_plugin.edl` file and generates the following files within the host directory:

| file | description |
|---|---|
| host/helloworld_args.h | Defines the parameters that are passed to all functions defined in the edl file |
| host/helloworld_u.c | Contains the `enclave_attestation_plugin()` function with the marshaling code to call into the enclave version of the `enclave_attestation_plugin()` function |
| host/helloworld_u.h | Function prototype for `enclave_attestation_plugin()` function |
...
| file | description |
|---|---|
| enclave/attestation_plugin_args.h | Defines the parameters that are passed to all functions defined in the edl file |
| enclave/attestation_plugin_t.c | Contains the `host_attestation_plugin()` function with the marshaling code to call into the host version of the `host_attestation_plugin()` function |
| enclave/attestation_plugin_t.h | function prototype for `host_attestation_plugin()` function |
...
This section shows how to develop and build a simple enclave called attestation_plugin.

### Develop an enclave

An enclave exposes its functionality to the host application in the form of a set of trusted methods that are defined in the `attestation_plugin.edl` file and implemented within the enclave project.

The attestation_plugin sample implements a single function named `enclave_attestation_plugin` which is called by the host. All it does is print out a message and then call back to the host. No parameters are passed in this sample for simplicity.

The full source for the enclave implementation is here: [attestation_plugin/enclave/enc.c](enclave/enc.c)
...
An enclave exposes its functionality via a set of methods defined in the `attestation_plugin.edl` file and implemented here. The only implemented function in the enclave in this sample is `enclave_attestation_plugin`.
...
This calls the marshaling function that is generated from the `attestation_plugin.edl` file which in turn calls into the function within the host. Even though the `host_attestation_plugin()` function is a `void` this call can still fail within the marshaling code itself and so we should always validate it. If `host_attestation_plugin()` were to return a value, it would actually be passed back as an out parameter of the function.
...
| File | Description |
| --- | --- |
| enc.c | Source code for the enclave `enclave_attestation_plugin` function |
| Makefile | Makefile used to build the enclave |
| attestation_plugin.conf | Configuration parameters for the enclave |
...
| attestation_plugin_denc | built and linked enclave executable |
| attestation_plugin_enc.signed | signed version of the enclave executable |
| attestation_plugin_args.h | Defines the parameters that are passed to all functions defined in the edl file |
| attestation_plugin_t.c | Contains the `host_attestation_plugin()` function with the marshaling code to call into the host version of the `host_attestation_plugin()` function |
| attestation_plugin_t.h | function prototype for `host_attestation_plugin()` function |
| attestation_plugin_t.o | compiled marshaling code for attestation_plugin_t.c |
...
Only the signed version of the enclave `attestation_plugin_enc.signed` is loadable on Linux as enclaves are required to be digitally signed.

#### Under the hood for the `make build` operation

Here is a listing of key components in the attestation_plugin/enclave/Makefile. Also see the [complete listing](enclave/Makefile).
...
build:
	...
	$(CC) -c $(CFLAGS) helloworld_t.c -o attestation_plugin_t.o
	$(CC) -o attestation_plugin_enc attestation_plugin_t.o enc.o $(LDFLAGS)

sign:
	oesign -e attestation_plugin_enc -c attestation_plugin.conf -k private.pem

clean:
	rm -f enc.o attestation_plugin_enc attestation_plugin_enc.signed private.pem ...
...
The signing process also reads the `attestation_plugin.conf` file which describes important parameters associated with with enclave.
...
In this section we will cover how to develop a host to load and run the attestation_plugin enclave we built above.
...
The full source for the host implementation is here: [attestation_plugin/host/host.c](host/host.c)
...
This is the actual host function that the enclave calls into. The function is defined in the `attestation_plugin.edl` file and implemented here.
...
The attestation_plugin sample creates an enclave by calling `oe_create_attestation_plugin_enclave` with the path to the signed enclave library file, which happens to be passed as the first parameter to the launching application.
...
This function calls into the host marshaling function that is generated from the `attestation_plugin.edl` file. It handles the code that marshals any parameters, and calls the function within the enclave itself. In this sample, we do not have any actual function parameters. Even though the function `enclave_attestation_plugin()` is a `void` return type, the marshaling code itself can fail, so we need to validate the return code associated with it. If `enclave_attestation_plugin()` were to return a value, this would be passed back as an out parameter.
...
The attestation_plugin sample comes with a Makefile with a `build` target. You can run `make build` to generate the marshaling files and build the host app.

Listing of [attestation_plugin/host/Makefile](host/Makefile)
...
build:
	...
	oeedger8r ../attestation_plugin.edl --untrusted
	$(CC) -c $(CFLAGS) host.c
	$(CC) -c $(CFLAGS) attestation_plugin_u.c
	$(CC) -o attestation_plugin_host attestation_plugin_u.o host.o $(LDFLAGS)

clean:
	rm -f attestation_plugin_host host.o attestation_plugin_u.o attestation_plugin_u.c attestation_plugin_u.h attestation_plugin_args.h
...
attestation_plugin sample can run under OE simulation mode.

To run the attestation_plugin sample in simulation mode from the command like, use the following:

```bash
./host/attestation_plugin_host ./enclave/attestation_plugin_enc.signed --simulate
```
