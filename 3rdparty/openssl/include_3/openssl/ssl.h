/*
 * WARNING: do not edit!
 * Generated by include/openssl/ssl.h from include/openssl/ssl.h.in
 *
...
/* see tls_srp.c */
    ...
    OSSL_DEPRECATEDIN_3_0 __owur int SSL_srp_server_param_with_username(
        SSL* s,
        int* ad);
...
#define SSL_SESSION_CACHE_MAX_SIZE_DEFAULT (1024 * 20)
     ...
     */
    typedef int (
        *GEN_SESSION_CB)(SSL* ssl, unsigned char* id, unsigned int* id_len);
...
#define SSL_CTX_sess_cache_full(ctx) \
    ...
    int (*SSL_CTX_sess_get_new_cb(
        SSL_CTX* ctx))(struct ssl_st* ssl, SSL_SESSION* sess);
    void SSL_CTX_sess_set_remove_cb(
        SSL_CTX* ctx,
        void (*remove_session_cb)(struct ssl_ctx_st* ctx, SSL_SESSION* sess));
    void (*SSL_CTX_sess_get_remove_cb(
        SSL_CTX* ctx))(struct ssl_ctx_st* ctx, SSL_SESSION* sess);
    ...
    void (*SSL_CTX_get_info_callback(
        SSL_CTX* ctx))(const SSL* ssl, int type, int val);
    void SSL_CTX_set_client_cert_cb(
        SSL_CTX* ctx,
        int (*client_cert_cb)(SSL* ssl, X509** x509, EVP_PKEY** pkey));
    int (*SSL_CTX_get_client_cert_cb(
        SSL_CTX* ctx))(SSL* ssl, X509** x509, EVP_PKEY** pkey);
...
#define OPENSSL_NPN_NO_OVERLAP 2
    ...
    __owur int SSL_CTX_set_alpn_protos(
        SSL_CTX* ctx,
        const unsigned char* protos,
        unsigned int protos_len);
    __owur int SSL_set_alpn_protos(
        SSL* ssl,
        const unsigned char* protos,
        unsigned int protos_len);
...
#define PSK_MAX_PSK_LEN 512
    ...
    __owur int SSL_CTX_use_psk_identity_hint(
        SSL_CTX* ctx,
    ...
    void SSL_set_psk_find_session_callback(
        SSL* s,
        SSL_psk_find_session_cb_func cb);
    void SSL_CTX_set_psk_find_session_callback(
        SSL_CTX* ctx,
        SSL_psk_find_session_cb_func cb);
    void SSL_set_psk_use_session_callback(
        SSL* s,
        SSL_psk_use_session_cb_func cb);
    void SSL_CTX_set_psk_use_session_callback(
        SSL_CTX* ctx,
        SSL_psk_use_session_cb_func cb);
    ...
    __owur int SSL_CTX_has_client_custom_ext(
        const SSL_CTX* ctx,
        unsigned int ext_type);
...
#define SSL_MAC_FLAG_WRITE_MAC_TLSTREE 8
    ...
    /*
     * SSL_CTX_set_keylog_callback configures a callback to log key material.
     * This is intended for debugging use with tools like Wireshark. The cb
     * function should log line followed by a newline.
     */
...
#define SSL_KEY_UPDATE_REQUESTED 1
    ...
    /*
     * The valid handshake states (one for each type message sent and one for
     * each type of message received). There are also two "special" states: TLS
     * = TLS or DTLS state DTLS = DTLS specific state CR/SR = Client Read/Server
     * Read CW/SW = Client Write/Server Write
     *
    ...
    /*
     * Most of the following state values are no longer used and are defined to
     * be the closest equivalent value in the current state machine code. Not
     * all defines have an equivalent and are set to a dummy value (-1).
     * SSL_ST_CONNECT and SSL_ST_ACCEPT are still in use in the definition of
     * SSL_CB_ACCEPT_LOOP, SSL_CB_ACCEPT_EXIT, SSL_CB_CONNECT_LOOP and
     * SSL_CB_CONNECT_EXIT.
     */
...
#define SSL_CTX_get0_implemented_groups(ctx, all, out) \
    SSL_CTX_ctrl(                                      \
        ctx,                                           \
        SSL_CTRL_GET0_IMPLEMENTED_GROUPS,              \
        all,                                           \
...
#define SSL_get0_signature_name(s, str) \
    SSL_ctrl(                           \
        s, SSL_CTRL_GET_SIGNATURE_NAME, 0, (1 ? (str) : (const char**)NULL))
...
#define SSL_set_tmp_rsa_callback(ssl, cb) \
    ...
    OSSL_DEPRECATEDIN_3_4_FOR(
        "not Y2038-safe, replace with SSL_CTX_flush_sessions_ex()")
    ...
    __owur int SSL_use_RSAPrivateKey_ASN1(
        SSL* ssl,
        const unsigned char* d,
        long len);
#endif
    __owur int SSL_use_PrivateKey(SSL* ssl, EVP_PKEY* pkey);
    __owur int SSL_use_PrivateKey_ASN1(
        int pk,
        SSL* ssl,
        const unsigned char* d,
        long len);
    __owur int SSL_use_certificate(SSL* ssl, X509* x);
    __owur int SSL_use_certificate_ASN1(
        SSL* ssl,
        const unsigned char* d,
        int len);
    __owur int SSL_use_cert_and_key(
        SSL* ssl,
        X509* x509,
        EVP_PKEY* privatekey,
        STACK_OF(X509) * chain,
...
#define SSL_SERVERINFOV2 2
    ...
    __owur int SSL_CTX_use_serverinfo(
        SSL_CTX* ctx,
        const unsigned char* serverinfo,
        size_t serverinfo_length);
    __owur int SSL_CTX_use_serverinfo_ex(
        SSL_CTX* ctx,
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
    OSSL_DEPRECATEDIN_3_0
    __owur int SSL_CTX_use_RSAPrivateKey_file(
        SSL_CTX* ctx,
        const char* file,
        int type);
#endif
    __owur int SSL_CTX_use_PrivateKey_file(
        SSL_CTX* ctx,
        const char* file,
        int type);
    __owur int SSL_CTX_use_certificate_file(
        SSL_CTX* ctx,
        const char* file,
        int type);
    /* PEM type */
    __owur int SSL_CTX_use_certificate_chain_file(
        SSL_CTX* ctx,
        const char* file);
    __owur int SSL_use_certificate_chain_file(SSL* ssl, const char* file);
    __owur STACK_OF(X509_NAME) * SSL_load_client_CA_file(const char* file);
    __owur STACK_OF(X509_NAME) * SSL_load_client_CA_file_ex(
                                     const char* file,
                                     OSSL_LIB_CTX* libctx,
                                     const char* propq);
    __owur int SSL_add_file_cert_subjects_to_stack(
        STACK_OF(X509_NAME) * stackCAs,
        const char* file);
    int SSL_add_dir_cert_subjects_to_stack(
        STACK_OF(X509_NAME) * stackCAs,
        const char* dir);
    int SSL_add_store_cert_subjects_to_stack(
        STACK_OF(X509_NAME) * stackCAs,
        const char* uri);
...
#define SSL_load_error_strings()                                          \
...
#ifndef OPENSSL_NO_DEPRECATED_3_4
    OSSL_DEPRECATEDIN_3_4_FOR(
        "not Y2038-safe, replace with SSL_SESSION_get_time_ex()")
    __owur long SSL_SESSION_get_time(const SSL_SESSION* s);
    OSSL_DEPRECATEDIN_3_4_FOR(
        "not Y2038-safe, replace with SSL_SESSION_set_time_ex()")
    ...
    void SSL_SESSION_get0_alpn_selected(
        ...
        size_t* len);
    __owur int SSL_SESSION_set1_alpn_selected(
        SSL_SESSION* s,
        const unsigned char* alpn,
        size_t len);
    ...
    __owur unsigned long SSL_SESSION_get_ticket_lifetime_hint(
        const SSL_SESSION* s);
    ...
    __owur int SSL_SESSION_set_max_early_data(
        SSL_SESSION* s,
        uint32_t max_early_data);
    __owur int SSL_copy_session_id(SSL* to, const SSL* from);
    __owur X509* SSL_SESSION_get0_peer(SSL_SESSION* s);
    __owur int SSL_SESSION_set1_id_context(
        SSL_SESSION* s,
        const unsigned char* sid_ctx,
        unsigned int sid_ctx_len);
    __owur int SSL_SESSION_set1_id(
        SSL_SESSION* s,
    ...
    __owur int SSL_has_matching_session_id(
        const SSL* s,
        const unsigned char* id,
        unsigned int id_len);
    SSL_SESSION* d2i_SSL_SESSION(
        ...
        long length);
    SSL_SESSION* d2i_SSL_SESSION_ex(
        SSL_SESSION** a,
        const unsigned char** pp,
        long length,
        OSSL_LIB_CTX* libctx,
...
#define SSL_get_peer_certificate SSL_get1_peer_certificate
    ...
    OSSL_DEPRECATEDIN_3_0
    __owur int SSL_CTX_use_RSAPrivateKey_ASN1(
        SSL_CTX* ctx,
        const unsigned char* d,
        long len);
#endif
    __owur int SSL_CTX_use_PrivateKey(SSL_CTX* ctx, EVP_PKEY* pkey);
    __owur int SSL_CTX_use_PrivateKey_ASN1(
        int pk,
        SSL_CTX* ctx,
        const unsigned char* d,
        long len);
    __owur int SSL_CTX_use_certificate(SSL_CTX* ctx, X509* x);
    __owur int SSL_CTX_use_certificate_ASN1(
        SSL_CTX* ctx,
        int len,
        const unsigned char* d);
    __owur int SSL_CTX_use_cert_and_key(
        SSL_CTX* ctx,
        X509* x509,
        EVP_PKEY* privatekey,
        STACK_OF(X509) * chain,
    ...
    __owur int SSL_set_session_id_context(
        SSL* ssl,
        const unsigned char* sid_ctx,
        unsigned int sid_ctx_len);
    ...
    __owur int SSL_CTX_dane_mtype_set(
        SSL_CTX* ctx,
        const EVP_MD* md,
        uint8_t mtype,
        uint8_t ord);
    __owur int SSL_dane_enable(SSL* s, const char* basedomain);
    __owur int SSL_dane_tlsa_add(
        SSL* s,
        uint8_t usage,
        uint8_t selector,
        uint8_t mtype,
    ...
    __owur int SSL_get0_dane_tlsa(
        ...
        size_t* dlen);
    /*
     * Bridge opacity barrier between libcrypt and libssl, also needed to
     * support offline testing in test/danetest.c
     */
...
#ifndef OPENSSL_NO_SRP
#ifndef OPENSSL_NO_DEPRECATED_3_0
    OSSL_DEPRECATEDIN_3_0 int SSL_CTX_set_srp_username(
        SSL_CTX* ctx,
        char* name);
    OSSL_DEPRECATEDIN_3_0 int SSL_CTX_set_srp_password(
        SSL_CTX* ctx,
        char* password);
    OSSL_DEPRECATEDIN_3_0 int SSL_CTX_set_srp_strength(
        SSL_CTX* ctx,
        int strength);
    OSSL_DEPRECATEDIN_3_0
...
#define SSL_CLIENT_HELLO_RETRY (-1)
    ...
    void SSL_CTX_set_client_hello_cb(
        ...
        void* arg);
    typedef int (
        *SSL_new_pending_conn_cb_fn)(SSL_CTX* ctx, SSL* new_ssl, void* arg);
    void SSL_CTX_set_new_pending_conn_cb(
        SSL_CTX* c,
    ...
    size_t SSL_client_hello_get0_compression_methods(
        SSL* s,
        const unsigned char** out);
    int SSL_client_hello_get1_extensions_present(
        ...
        int** out,
        size_t* outlen);
    int SSL_client_hello_get_extension_order(
        SSL* s,
        uint16_t* exts,
        size_t* num_exts);
    ...
    __owur int SSL_get_all_async_fds(
        SSL* s,
        OSSL_ASYNC_FD* fds,
        size_t* numfds);
    __owur int SSL_get_changed_async_fds(
        SSL* s,
        OSSL_ASYNC_FD* addfd,
        size_t* numaddfds,
        OSSL_ASYNC_FD* delfd,
        size_t* numdelfds);
    __owur int SSL_CTX_set_async_callback(
        SSL_CTX* ctx,
...
#define SSL_READ_EARLY_DATA_FINISH 2
    ...
    __owur int SSL_read_early_data(
        SSL* s,
        void* buf,
        size_t num,
        size_t* readbytes);
    ...
    __owur ossl_ssize_t
    SSL_sendfile(SSL* s, int fd, off_t offset, size_t size, int flags);
    __owur int SSL_write(SSL* ssl, const void* buf, int num);
    __owur int SSL_write_ex(
        SSL* s,
        const void* buf,
        size_t num,
        size_t* written);
...
#ifndef OPENSSL_NO_DEPRECATED_1_1_0
    OSSL_DEPRECATEDIN_1_1_0 __owur const SSL_METHOD* SSLv3_method(
        void); /* SSLv3 */
...
#ifndef OPENSSL_NO_DEPRECATED_1_1_0
    OSSL_DEPRECATEDIN_1_1_0 __owur const SSL_METHOD* TLSv1_method(
        void); /* TLSv1.0 */
...
#ifndef OPENSSL_NO_DEPRECATED_1_1_0
    OSSL_DEPRECATEDIN_1_1_0 __owur const SSL_METHOD* TLSv1_1_method(
        void); /* TLSv1.1 */
    OSSL_DEPRECATEDIN_1_1_0 __owur const SSL_METHOD* TLSv1_1_server_method(
        void);
    OSSL_DEPRECATEDIN_1_1_0 __owur const SSL_METHOD* TLSv1_1_client_method(
        void);
#endif
#endif
...
#ifndef OPENSSL_NO_DEPRECATED_1_1_0
    OSSL_DEPRECATEDIN_1_1_0 __owur const SSL_METHOD* TLSv1_2_method(
        void); /* TLSv1.2 */
    OSSL_DEPRECATEDIN_1_1_0 __owur const SSL_METHOD* TLSv1_2_server_method(
        void);
    OSSL_DEPRECATEDIN_1_1_0 __owur const SSL_METHOD* TLSv1_2_client_method(
        void);
#endif
#endif
...
/* DTLSv1.2 */
    ...
    OSSL_DEPRECATEDIN_1_1_0 __owur const SSL_METHOD* DTLSv1_2_server_method(
        void);
    OSSL_DEPRECATEDIN_1_1_0 __owur const SSL_METHOD* DTLSv1_2_client_method(
        void);
#endif
#endif
...
#define SSL_library_init() OPENSSL_init_ssl(0, NULL)
    ...
    __owur int SSL_CTX_load_verify_locations(
        SSL_CTX* ctx,
        const char* CAfile,
        const char* CApath);
#define SSL_get0_session SSL_get_session /* just peek at pointer */
    __owur SSL_SESSION* SSL_get_session(const SSL* ssl);
    __owur SSL_SESSION* SSL_get1_session(
    ...
    void (*SSL_get_info_callback(
        const SSL* ssl))(const SSL* ssl, int type, int val);
    ...
    __owur size_t SSL_SESSION_get_master_key(
        const SSL_SESSION* sess,
        unsigned char* out,
        size_t outlen);
    __owur int SSL_SESSION_set1_master_key(
        SSL_SESSION* sess,
        const unsigned char* in,
        size_t len);
...
#define SSL_SESSION_get_ex_new_index(l, p, newf, dupf, freef) \
    CRYPTO_get_ex_new_index(                                  \
        CRYPTO_EX_INDEX_SSL_SESSION, l, p, newf, dupf, freef)
...
#define SSL_COMP_free_compression_methods() \
    ...
    __owur int SSL_set_session_ticket_ext_cb(
        SSL* s,
        tls_session_ticket_ext_cb_fn cb,
        void* arg);

    /* Pre-shared secret session resumption functions */
    __owur int SSL_set_session_secret_cb(
        SSL* s,
    ...
    void SSL_CTX_set_record_padding_callback(
        SSL_CTX* ctx,
        size_t (*cb)(SSL* ssl, int type, size_t len, void* arg));
    ...
    int SSL_set_record_padding_callback(
        SSL* ssl,
        size_t (*cb)(SSL* ssl, int type, size_t len, void* arg));
    void SSL_set_record_padding_callback_arg(SSL* ssl, void* arg);
    void* SSL_get_record_padding_callback_arg(const SSL* ssl);
    int SSL_set_block_padding(SSL* ssl, size_t block_size);
    int SSL_set_block_padding_ex(
        SSL* ssl,
        size_t app_block_size,
        size_t hs_block_size);
    ...
    __owur int SSL_get_event_timeout(
        SSL* s,
        struct timeval* tv,
        int* is_infinite);
...
#define SSL_DOMAIN_FLAG_LEGACY_BLOCKING (1U << 4)
    ...
    __owur int SSL_CTX_get_domain_flags(
        const SSL_CTX* ctx,
...
#define SSL_ACCEPT_STREAM_NO_BLOCK (1U << 0)
    ...
    __owur int SSL_inject_net_dgram(
        SSL* s,
...
#define SSL_STREAM_STATE_CONN_CLOSED 6
    ...
    __owur int SSL_get_stream_read_error_code(
        SSL* ssl,
        uint64_t* app_error_code);
    __owur int SSL_get_stream_write_error_code(
        SSL* ssl,
        uint64_t* app_error_code);
...
#define SSL_CONN_CLOSE_FLAG_TRANSPORT (1U << 1)
    ...
    __owur int SSL_get_conn_close_info(
        SSL* ssl,
...
#define SSL_get_quic_stream_bidi_local_avail(ssl, value) \
    SSL_get_generic_value_uint(                          \
        (ssl), SSL_VALUE_QUIC_STREAM_BIDI_LOCAL_AVAIL, (value))
#define SSL_get_quic_stream_bidi_remote_avail(ssl, value) \
    SSL_get_generic_value_uint(                           \
        (ssl), SSL_VALUE_QUIC_STREAM_BIDI_REMOTE_AVAIL, (value))
#define SSL_get_quic_stream_uni_local_avail(ssl, value) \
    SSL_get_generic_value_uint(                         \
        (ssl), SSL_VALUE_QUIC_STREAM_UNI_LOCAL_AVAIL, (value))
#define SSL_get_quic_stream_uni_remote_avail(ssl, value) \
    SSL_get_generic_value_uint(                          \
        (ssl), SSL_VALUE_QUIC_STREAM_UNI_REMOTE_AVAIL, (value))
...
#define SSL_POLL_EVENT_E                                         \
    (SSL_POLL_EVENT_EL | SSL_POLL_EVENT_EC | SSL_POLL_EVENT_ER | \
     SSL_POLL_EVENT_EW)
...
#define SSL_cache_hit(s) SSL_session_reused(s)
    ...
    __owur unsigned int SSL_CONF_CTX_clear_flags(
        SSL_CONF_CTX* cctx,
        unsigned int flags);
    ...
    /*
     * Sets a |callback| that is invoked upon receipt of ServerHelloDone to
     * validate the received SCTs. If the callback returns a non-positive
     * result, the connection is terminated. Call this function before beginning
     * a handshake. If a NULL |callback| is provided, SCT validation is
     * disabled. |arg| is arbitrary userdata that will be passed to the callback
     * whenever it is invoked. Ownership of |arg| remains with the caller.
     *
     * NOTE: A side-effect of setting a CT callback is that an OCSP stapled
     * response will be requested.
     */
    int SSL_set_ct_validation_callback(
        SSL* s,
        ssl_ct_validation_cb callback,
        void* arg);
    int SSL_CTX_set_ct_validation_callback(
        SSL_CTX* ctx,
        ssl_ct_validation_cb callback,
        void* arg);
...
#define SSL_CTX_disable_ct(ctx) \
    ...
    /*
     * The validation type enumerates the available behaviours of the built-in
     * SSL CT validation callback selected via SSL_enable_ct() and
     * SSL_CTX_enable_ct(). The underlying callback is a static function in
     * libssl.
     */
    enum
    {
     ...
     * continues the handshake, the application can make appropriate decisions
     * at handshake completion.  The SSL_CT_VALIDATION_STRICT variant requires
     * at least one valid SCT, or else handshake termination will be requested.
     * The handshake may continue anyway if SSL_VERIFY_NONE is in effect.
     */
    ...
    /*
     * Sets the CT log list used by all SSL connections created from this
     * SSL_CTX. Ownership of the CTLOG_STORE is transferred to the SSL_CTX.
     */
    void SSL_CTX_set0_ctlog_store(SSL_CTX* ctx, CTLOG_STORE* logs);

    /*
     * Gets the CT log list used by all SSL connections created from this
     * SSL_CTX. This will be NULL unless one of the following functions has been
     * called:
...
#define SSL_SECOP_PEER_CA_MD (SSL_SECOP_CA_MD | SSL_SECOP_PEER)
    ...
    void SSL_set_security_callback(
        SSL* s,
        int (*cb)(
            const SSL* s,
            const SSL_CTX* ctx,
    ...
    void SSL_CTX_set_security_callback(
        SSL_CTX* ctx,
        int (*cb)(
            const SSL* s,
            const SSL_CTX* ctx,
...
#define SSL_TICKET_RETURN_USE_RENEW 4
    ...
    typedef SSL_TICKET_RETURN (*SSL_CTX_decrypt_session_ticket_fn)(
        ...
        void* arg);
    int SSL_CTX_set_session_ticket_cb(
        SSL_CTX* ctx,
        SSL_CTX_generate_session_ticket_fn gen_cb,
        SSL_CTX_decrypt_session_ticket_fn dec_cb,
        void* arg);
    int SSL_SESSION_set1_ticket_appdata(
        SSL_SESSION* ss,
        const void* data,
        size_t len);
    int SSL_SESSION_get0_ticket_appdata(
        SSL_SESSION* ss,
        void** data,
        size_t* len);
    ...
    void SSL_CTX_set_allow_early_data_cb(
        ...
        void* arg);
    void SSL_set_allow_early_data_cb(
        SSL* s,
        SSL_allow_early_data_cb_fn cb,
        void* arg);
    ...
    int SSL_CTX_set1_compressed_cert(
        SSL_CTX* ctx,
        int algorithm,
        unsigned char* comp_data,
        size_t comp_length,
        size_t orig_length);
    int SSL_set1_compressed_cert(
        SSL* ssl,
        int algorithm,
        unsigned char* comp_data,
        size_t comp_length,
        size_t orig_length);
    size_t SSL_CTX_get1_compressed_cert(
        SSL_CTX* ctx,
        int alg,
        unsigned char** data,
        size_t* orig_len);
    size_t SSL_get1_compressed_cert(
        SSL* ssl,
        int alg,
        unsigned char** data,
        size_t* orig_len);
    ...
    __owur int SSL_set1_client_cert_type(
        SSL* s,
        const unsigned char* val,
        size_t len);
    __owur int SSL_set1_server_cert_type(
        SSL* s,
        const unsigned char* val,
        size_t len);
    __owur int SSL_CTX_set1_client_cert_type(
        SSL_CTX* ctx,
        const unsigned char* val,
        size_t len);
    __owur int SSL_CTX_set1_server_cert_type(
        SSL_CTX* ctx,
        const unsigned char* val,
        size_t len);
    __owur int SSL_get0_client_cert_type(
        const SSL* s,
        unsigned char** t,
        size_t* len);
    __owur int SSL_get0_server_cert_type(
        const SSL* s,
        unsigned char** t,
        size_t* len);
    __owur int SSL_CTX_get0_client_cert_type(
        const SSL_CTX* ctx,
        unsigned char** t,
        size_t* len);
    __owur int SSL_CTX_get0_server_cert_type(
        const SSL_CTX* s,
        unsigned char** t,
        size_t* len);
...
#define OSSL_RECORD_PROTECTION_LEVEL_APPLICATION 3
    ...
    int SSL_set_quic_tls_transport_params(
        SSL* s,
