...
extern "C"
{
    ...
    typedef int (*OPENSSL_LH_COMPFUNCTHUNK)(
        const void*,
        const void*,
        OPENSSL_LH_COMPFUNC cfn);
    typedef unsigned long (*OPENSSL_LH_HASHFUNC)(const void*);
    typedef unsigned long (
        *OPENSSL_LH_HASHFUNCTHUNK)(const void*, OPENSSL_LH_HASHFUNC hfn);
    typedef void (*OPENSSL_LH_DOALL_FUNC)(void*);
    typedef void (
        *OPENSSL_LH_DOALL_FUNC_THUNK)(void*, OPENSSL_LH_DOALL_FUNC doall);
    typedef void (*OPENSSL_LH_DOALL_FUNCARG)(void*, void*);
    typedef void (*OPENSSL_LH_DOALL_FUNCARG_THUNK)(
        void*,
        void*,
        OPENSSL_LH_DOALL_FUNCARG doall);
...
#define LH_LOAD_MULT 256
    ...
    void OPENSSL_LH_doall_arg(
        ...
        void* arg);
    void OPENSSL_LH_doall_arg_thunk(
        OPENSSL_LHASH* lh,
        OPENSSL_LH_DOALL_FUNCARG_THUNK daaw,
    ...
    OSSL_DEPRECATEDIN_3_1 void OPENSSL_LH_stats(
        const OPENSSL_LHASH* lh,
        FILE* fp);
    OSSL_DEPRECATEDIN_3_1 void OPENSSL_LH_node_stats(
        const OPENSSL_LHASH* lh,
        FILE* fp);
    OSSL_DEPRECATEDIN_3_1 void OPENSSL_LH_node_usage_stats(
        const OPENSSL_LHASH* lh,
        FILE* fp);
#endif
#endif
#ifndef OPENSSL_NO_DEPRECATED_3_1
    OSSL_DEPRECATEDIN_3_1 void OPENSSL_LH_stats_bio(
        const OPENSSL_LHASH* lh,
        BIO* out);
    OSSL_DEPRECATEDIN_3_1 void OPENSSL_LH_node_stats_bio(
        const OPENSSL_LHASH* lh,
        BIO* out);
    OSSL_DEPRECATEDIN_3_1 void OPENSSL_LH_node_usage_stats_bio(
        const OPENSSL_LHASH* lh,
        BIO* out);
#endif
...
#define DEFINE_LHASH_OF_INTERNAL(type)                                         \
    ...
    static ossl_inline unsigned long lh_##type##_hash_thunk(                   \
        const void* data, OPENSSL_LH_HASHFUNC hfn)                             \
    {                                                                          \
        unsigned long (*hfn_conv)(const type*) =                               \
            (unsigned long (*)(const type*))hfn;                               \
        return hfn_conv((const type*)data);                                    \
    }                                                                          \
    static ossl_inline int lh_##type##_comp_thunk(                             \
        const void* da, const void* db, OPENSSL_LH_COMPFUNC cfn)               \
    {                                                                          \
        int (*cfn_conv)(const type*, const type*) =                            \
            (int (*)(const type*, const type*))cfn;                            \
        return cfn_conv((const type*)da, (const type*)db);                     \
    }                                                                          \
    static ossl_inline void lh_##type##_doall_thunk(                           \
        void* node, OPENSSL_LH_DOALL_FUNC doall)                               \
    {                                                                          \
        void (*doall_conv)(type*) = (void (*)(type*))doall;                    \
        doall_conv((type*)node);                                               \
    }                                                                          \
    static ossl_inline void lh_##type##_doall_arg_thunk(                       \
        void* node, void* arg, OPENSSL_LH_DOALL_FUNCARG doall)                 \
    {                                                                          \
        void (*doall_conv)(type*, void*) = (void (*)(type*, void*))doall;      \
        doall_conv((type*)node, arg);                                          \
    }                                                                          \
    static ossl_unused ossl_inline type* ossl_check_##type##_lh_plain_type(    \
        type* ptr)                                                             \
    {                                                                          \
        return ptr;                                                            \
    }                                                                          \
    ...
    static ossl_unused ossl_inline OPENSSL_LHASH* ossl_check_##type##_lh_type( \
        LHASH_OF(type) * lh)                                                   \
    {                                                                          \
...
#define DEFINE_LHASH_OF_EX(type)                                               \
    ...
    static unsigned long lh_##type##_hfn_thunk(                                \
        const void* data, OPENSSL_LH_HASHFUNC hfn)                             \
    {                                                                          \
        unsigned long (*hfn_conv)(const type*) =                               \
            (unsigned long (*)(const type*))hfn;                               \
    ...
    static int lh_##type##_cfn_thunk(                                          \
        const void* da, const void* db, OPENSSL_LH_COMPFUNC cfn)               \
    {                                                                          \
        int (*cfn_conv)(const type*, const type*) =                            \
            (int (*)(const type*, const type*))cfn;                            \
    ...
    static ossl_unused ossl_inline void lh_##type##_doall_thunk(               \
        void* node, OPENSSL_LH_DOALL_FUNC doall)                               \
    {                                                                          \
        void (*doall_conv)(type*) = (void (*)(type*))doall;                    \
        doall_conv((type*)node);                                               \
    }                                                                          \
    static ossl_unused ossl_inline void lh_##type##_doall_arg_thunk(           \
        void* node, void* arg, OPENSSL_LH_DOALL_FUNCARG doall)                 \
    {                                                                          \
        void (*doall_conv)(type*, void*) = (void (*)(type*, void*))doall;      \
        doall_conv((type*)node, arg);                                          \
    ...
    static ossl_unused ossl_inline LHASH_OF(type) *                            \
        lh_##type##_new(                                                       \
            unsigned long (*hfn)(const type*),                                 \
            int (*cfn)(const type*, const type*))                              \
    {                                                                          \
        return (LHASH_OF(type)*)OPENSSL_LH_set_thunks(                         \
            OPENSSL_LH_new(                                                    \
    ...
    static ossl_unused ossl_inline void lh_##type##_doall_arg(                 \
        LHASH_OF(type) * lh, void (*doallarg)(type*, void*), void* arg)        \
    {                                                                          \
        OPENSSL_LH_doall_arg(                                                  \
            (OPENSSL_LHASH*)lh, (OPENSSL_LH_DOALL_FUNCARG)doallarg, arg);      \
    }                                                                          \
    LHASH_OF(type)
...
#define int_implement_lhash_doall(type, argtype, cbargtype)                  \
    static ossl_unused ossl_inline void lh_##type##_doall_##argtype##_thunk( \
        void* node, void* arg, OPENSSL_LH_DOALL_FUNCARG fn)                  \
    {                                                                        \
        void (*fn_conv)(cbargtype*, argtype*) =                              \
            (void (*)(cbargtype*, argtype*))fn;                              \
