/*
 * WARNING: do not edit!
 * Generated by include/openssl/ocsp.h from include/openssl/ocsp.h.in
 *
...
#define OCSP_CERTSTATUS_dup(cs)            \
    (OCSP_CERTSTATUS*)ASN1_dup(            \
        (i2d_of_void*)i2d_OCSP_CERTSTATUS, \
        (d2i_of_void*)d2i_OCSP_CERTSTATUS, \
        (char*)(cs))
    ...
    OSSL_HTTP_REQ_CTX* OCSP_sendreq_new(
        ...
        int buf_size);
    OCSP_RESPONSE* OCSP_sendreq_bio(
        BIO* b,
        const char* path,
        OCSP_REQUEST* req);
...
#define OCSP_REQ_CTX_http(rctx, op, path)                              \
    (OSSL_HTTP_REQ_CTX_set_expected(rctx, NULL, 1 /* asn1 */, 0, 0) && \
     OSSL_HTTP_REQ_CTX_set_request_line(                               \
         rctx, strcmp(op, "POST") == 0, NULL, NULL, path))
...
#define OCSP_sendreq_nbio(p, r) \
    OSSL_HTTP_REQ_CTX_nbio_d2i( \
        r, (ASN1_VALUE**)(p), ASN1_ITEM_rptr(OCSP_RESPONSE))
...
#define OCSP_set_max_response_length OSSL_HTTP_REQ_CTX_set_max_response_length
    ...
    int OCSP_resp_get0_id(
        const OCSP_BASICRESP* bs,
        const ASN1_OCTET_STRING** pid,
        const X509_NAME** pname);
    int OCSP_resp_get1_id(
        const OCSP_BASICRESP* bs,
...
#define OCSP_parse_url(url, host, port, path, ssl) \
    ...
    int OCSP_RESPID_set_by_key_ex(
        OCSP_RESPID* respid,
        X509* cert,
        OSSL_LIB_CTX* libctx,
        const char* propq);
    int OCSP_RESPID_set_by_key(OCSP_RESPID* respid, X509* cert);
    int OCSP_RESPID_match_ex(
        OCSP_RESPID* respid,
        X509* cert,
        OSSL_LIB_CTX* libctx,
        const char* propq);
    ...
    X509_EXTENSION* OCSP_url_svcloc_new(
        const X509_NAME* issuer,
        const char** urls);

    int OCSP_REQUEST_get_ext_count(OCSP_REQUEST* x);
    int OCSP_REQUEST_get_ext_by_NID(OCSP_REQUEST* x, int nid, int lastpos);
    int OCSP_REQUEST_get_ext_by_OBJ(
        OCSP_REQUEST* x,
        const ASN1_OBJECT* obj,
        int lastpos);
    int OCSP_REQUEST_get_ext_by_critical(
        OCSP_REQUEST* x,
        int crit,
        int lastpos);
    ...
    int OCSP_ONEREQ_get_ext_by_OBJ(
        OCSP_ONEREQ* x,
        const ASN1_OBJECT* obj,
        int lastpos);
    ...
    int OCSP_SINGLERESP_get_ext_by_NID(
        OCSP_SINGLERESP* x,
        int nid,
        int lastpos);
