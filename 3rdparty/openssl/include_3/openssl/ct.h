/*
 * WARNING: do not edit!
 * Generated by include/openssl/ct.h from include/openssl/ct.h.in
 *
...
#define sk_CTLOG_set_cmp_func(sk, cmp) ((sk_CTLOG_compfunc)OPENSSL_sk_set_cmp_func(ossl_check_CTLOG_sk_type(sk), ossl_check_CTLOG_compfunc_type(cmp)))
     ...
     * The caller is responsible for calling CT_POLICY_EVAL_CTX_free when
     * finished with the CT_POLICY_EVAL_CTX.
     */
    CT_POLICY_EVAL_CTX* CT_POLICY_EVAL_CTX_new_ex(
        OSSL_LIB_CTX* libctx,
        const char* propq);
    ...
    /* Sets the log store that is in use. It must outlive the
     * CT_POLICY_EVAL_CTX. */
    void CT_POLICY_EVAL_CTX_set_shared_CTLOG_STORE(
        CT_POLICY_EVAL_CTX* ctx,
        CTLOG_STORE* log_store);

    /*
     * Gets the time, in milliseconds since the Unix epoch, that will be used as
     * the current time when checking whether an SCT was issued in the future.
     * Such SCTs will fail validation, as required by RFC6962.
     */
    uint64_t CT_POLICY_EVAL_CTX_get_time(const CT_POLICY_EVAL_CTX* ctx);

    /*
     * Sets the time to evaluate SCTs against, in milliseconds since the Unix
     * epoch. If an SCT's timestamp is after this time, it will be interpreted
     * as having been issued in the future. RFC6962 states that "TLS clients
     * MUST reject SCTs whose timestamp is in the future", so an SCT will not
     ...
     * The caller is responsible for calling SCT_free when finished with the
     * SCT.
     */
    SCT* SCT_new(void);
    ...
    /*
     * Set *ext to point to the extension data for the SCT. ext must not be
     * NULL. The SCT retains ownership of this pointer. Returns length of the
     * data pointed to.
     */
     ...
     * Returns 1 on success, 0 otherwise.
     */
    __owur int SCT_set1_extensions(
        SCT* sct,
     ...
     * If |logs| is not NULL, it will be used to lookup the CT log that the SCT
     * came from, so that the log name can be printed.
     */
    void SCT_print(
        const SCT* sct,
        BIO* out,
     ...
     * "a" must not be NULL.
     * If "pp" is NULL, just return the length of what would have been
     * serialized. If "pp" is not NULL and "*pp" is null, function will allocate
     * a new pointer for data that caller is responsible for freeing (only if
     * function returns successfully). If "pp" is NULL and "*pp" is not NULL,
     * caller is responsible for ensuring that "*pp" is large enough to accept
     * all of the serialized data. Returns < 0 on error, >= 0 indicating bytes
     * written (or would have been) on success.
     */
     ...
     * will be returned.
     * Upon failure, a NULL pointer will be returned, and the position of "*pp"
     * is not defined.
     */
    STACK_OF(SCT) *
     ...
     * "a" must not be NULL.
     * If "pp" is NULL, just returns the length of what would have been
     * serialized. If "pp" is not NULL and "*pp" is null, function will allocate
     * a new pointer for data that caller is responsible for freeing (only if
     * function returns successfully). If "pp" is NULL and "*pp" is not NULL,
     * caller is responsible for ensuring that "*pp" is large enough to accept
     ...
     * returned. If |out| points to a null pointer, a string will be allocated
     * to hold the TLS-format SCT. It is the responsibility of the caller to
     * free it. If |out| points to an allocated string, the TLS-format SCT will
     * be written to it. The length of the SCT in TLS format will be returned.
     */
     ...
     * Returns NULL if malloc fails or if |public_key| cannot be converted to
     * DER. Should be deleted by the caller using CTLOG_free when no longer
     * needed.
     */
    CTLOG* CTLOG_new_ex(
        EVP_PKEY* public_key,
     ...
     * DER in |pkey_base64| and associated with the given library context
     * |libctx| and property query string |propq|. The |name| is a string to
     * help users identify this log. Returns 1 on success, 0 on failure. Should
     * be deleted by the caller using CTLOG_free when no longer needed.
     */
     ...
     * Should be deleted by the caller using CTLOG_STORE_free when no longer
     * needed.
     */
