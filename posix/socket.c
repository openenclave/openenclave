...
int oe_connect(int sockfd, const struct oe_sockaddr* addr, oe_socklen_t addrlen)
{
    ...
    ret = sock->ops.sock.connect(sock, addr, addrlen);
...
int oe_accept(int sockfd, struct oe_sockaddr* addr, oe_socklen_t* addrlen)
{
    ...
    if ((new_sock = sock->ops.sock.accept(sock, addr, addrlen)) == NULL)
        OE_RAISE_ERRNO(oe_errno);
...
int oe_listen(int sockfd, int backlog)
{
    ...
    ret = sock->ops.sock.listen(sock, backlog);
...
ssize_t oe_recv(int sockfd, void* buf, size_t len, int flags)
{
    ...
    ret = sock->ops.sock.recv(sock, buf, len, flags);
    ...
    oe_socklen_t* addrlen)
{
    ...
    ret = sock->ops.sock.recvfrom(sock, buf, len, flags, src_addr, addrlen);
...
ssize_t oe_send(int sockfd, const void* buf, size_t len, int flags)
{
    ...
    ret = sock->ops.sock.send(sock, buf, len, flags);
    ...
    oe_socklen_t addrlen)
{
    ...
    ret = sock->ops.sock.sendto(sock, buf, len, flags, dest_addr, addrlen);
...
ssize_t oe_recvmsg(int sockfd, struct oe_msghdr* buf, int flags)
{
    ...
    ret = sock->ops.sock.recvmsg(sock, buf, flags);
...
ssize_t oe_sendmsg(int sockfd, const struct oe_msghdr* buf, int flags)
{
    ...
    ret = sock->ops.sock.sendmsg(sock, buf, flags);
...
int oe_shutdown(int sockfd, int how)
{
    ...
    ret = sock->ops.sock.shutdown(sock, how);
...
int oe_getsockname(int sockfd, struct oe_sockaddr* addr, oe_socklen_t* addrlen)
{
    ...
    ret = sock->ops.sock.getsockname(sock, addr, addrlen);
...
int oe_getpeername(int sockfd, struct oe_sockaddr* addr, oe_socklen_t* addrlen)
{
    ...
    ret = sock->ops.sock.getpeername(sock, addr, addrlen);
    ...
    oe_socklen_t* optlen)
{
    ...
    ret = sock->ops.sock.getsockopt(sock, level, optname, optval, optlen);
    ...
    oe_socklen_t optlen)
{
    ...
    ret = sock->ops.sock.setsockopt(sock, level, optname, optval, optlen);
...
int oe_bind(int sockfd, const struct oe_sockaddr* name, oe_socklen_t namelen)
{
    ...
    ret = sock->ops.sock.bind(sock, name, namelen);
